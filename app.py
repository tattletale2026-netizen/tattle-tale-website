...
BOOKING_LIMIT = int(os.environ.get('BOOKING_LIMIT', 20))
BOOKING_DATA_PATH = os.environ.get(
    'BOOKING_DATA_PATH',
    os.path.join(app.root_path, 'data', 'bookings.json')
)

def _ensure_booking_file():
    os.makedirs(os.path.dirname(BOOKING_DATA_PATH), exist_ok=True)
    if not os.path.exists(BOOKING_DATA_PATH):
        with open(BOOKING_DATA_PATH, 'w') as f:
            json.dump({"count": 0, "bookings": []}, f)

def _load_booking_data():
    _ensure_booking_file()
    try:
        with open(BOOKING_DATA_PATH, 'r') as f:
            data = json.load(f)
            if isinstance(data, dict):
                data.setdefault("count", len(data.get("bookings", [])))
                data.setdefault("bookings", [])
                return data
    except Exception as e:
        print(f"Booking file error: {repr(e)}")
    return {"count": 0, "bookings": []}

def get_booking_count():
    return int(_load_booking_data().get("count", 0))

def save_booking(first_name, last_name, email, phone):
    data = _load_booking_data()
    current_count = int(data.get("count", 0))
    if current_count >= BOOKING_LIMIT:
    ...
    data.setdefault("bookings", []).append(booking_record)
...
def booking():
    ...
    closed = seats_taken >= BOOKING_LIMIT
    ...
    if request.method == 'POST':
        ...
        saved, updated_count = save_booking(first_name, last_name, email, phone)
        if not saved:
            return render_template("booking.html", closed=True)
