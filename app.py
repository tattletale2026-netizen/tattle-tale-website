...
MAIL_TO = os.getenv('MAIL_TO', os.getenv('MAIL_DEFAULT_SENDER'))
...
def survey():
    if request.method == 'POST':
        ...
        missing_config = email_config_ready()
        if missing_config:
            app.logger.error(f"Email config missing: {', '.join(missing_config)}")
            return render_template("newform.html", error="Email system is not configured correctly.")
...
def booking():
    ...
    if request.method == 'POST':
        ...
        if missing_config:
            app.logger.error(f"Email config missing: {', '.join(missing_config)}")
            return render_template("booking.html", closed=closed, error="Email system is not configured correctly.")
        ...
        try:
            ...
            app.logger.exception("Email sending failed")
            print(f"Mail error: {repr(e)}")
            return render_template("booking.html", closed=closed, error="Failed to send email. Please try again later.")
...
def handle_feedback():
    ...
    if missing_config:
        app.logger.error(f"Email config missing: {', '.join(missing_config)}")
        return jsonify({'success': False, 'message': 'Email system is not configured correctly.'}), 500
